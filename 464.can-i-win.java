...
class Solution {
    public boolean canIWin(int maxChoosableInteger, int desiredTotal) {
        return backtrack(maxChoosableInteger, desiredTotal, true, 0, new HashSet<Integer>());
    }

    private boolean backtrack(int max, int total, Set<Integer> used) {
        

        for (int i = max; i > 0; i--) {
            if (!used.contains(i)) {
                used.add(i);
                if (backtrack(max, total, !self, current + i, used))
                    return true;
                used.remove(i);
            }
        }
